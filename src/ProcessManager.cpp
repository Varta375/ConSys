...
const std::vector<ProcessInfo>& ProcessManager::Refresh() {
    ...
    try {
        ...
        for (bool more = Process32FirstW(Snapshot, &pe);
            more; // Контролирует наличие следующего элемента
            more = Process32NextW(Snapshot, &pe))
        {
            // Двухэтапная конвертация UTF-16 (wchar_t) в UTF-8 (std::string):
            int len = WideCharToMultiByte(CP_UTF8, 0, pe.szExeFile, -1, nullptr, 0, nullptr, nullptr); // Возвращает необходимый размер буфера (включая '\0')

            std::string name(len - 1, '\0');

            WideCharToMultiByte(CP_UTF8, 0, pe.szExeFile, -1, name.data(), len, nullptr, nullptr);

            if (name == "svchost.exe" || name == "dllhost.exe" || name == "ConSys.exe" || name == "ApplicationFrameHost.exe") {
                continue;
            }

            HANDLE hProc = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, pe.th32ProcessID); // Открываем процесс для проверки на его системность
            if (!hProc)
            {
                continue;
            }
...
bool ProcessManager::StopProcess(DWORD pid) {
    ...
    try {
        ...
        auto it = std::find_if(Processes.begin(), Processes.end(), [pid](const ProcessInfo& p) {return p.pid == pid;});
        if (it == Processes.end() || it->isSystem) return false;
